...
def get_ratings_range(lower_value: float, higher_value: float) -> pd.DataFrame:
    df = movie_df['my_rating'].between(lower_value, higher_value, inclusive=True)
    range_values = movie_df[df]
    return range_values


def select_date_range(start_date: datetime.datetime, end_date: datetime.datetime) -> (pd.DataFrame, pd.DataFrame):
...
def calculate_runtime(imdb_period: pd.DataFrame) -> str:
...
def count_item(imdb_period: pd.DataFrame, column: str) -> Counter:
    list_values = imdb_period[column].to_list()
    flattened_list = itertools.chain(*list_values)
    return Counter(flattened_list)


def ratings_genre(imdb_period: pd.DataFrame, movie_period: pd.DataFrame) -> OrderedDict:
...
if __name__ == '__main__':
    ...
    print(type(select_date_range(start_date, end_date)))
    print(type(calculate_runtime(imdb)))
    print(type(imdb))
    ...
    # print(type(get_ratings_range(3.5, 5)))
    print(type(select_date_range(start_date, end_date)))
    print(help())
